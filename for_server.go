...
func postCityHandler(c echo.Context) error {
	...
	_, err = db.Exec(fmt.Sprintf("insert into city (name, countrycode, district, population) values ('%v', '%v', '%v', '%v')", data.Name, data.CountryCode, data.District, data.Population))
	if err != nil {
		log.Fatalf("DB Error: %s", err)
	}
